...
class ASP:
        ...
        def add_sparse_attributes(module_name, module):
            ...
            for p_name, p in module.named_parameters():
                if p_name in sparse_parameters and p.requires_grad:
                    ...
                    if p.dtype == torch.float32 and ((p.size()[0] % 8) != 0 or (p.size()[1] % 8) != 0): #User defines FP32 and APEX internally uses FP16 math
                        print("[ASP] Auto skipping pruning %s::%s of size=%s and type=%s for sparsity" % (module_name, p_name, str(p.size()), str(p.dtype)))
                        continue
                    if p.dtype == torch.float16 and ((p.size()[0] % 8) != 0 or (p.size()[1] % 8) != 0): #For Conv2d dim= K x CRS; we prune along C
                        print("[ASP] Auto skipping pruning %s::%s of size=%s and type=%s for sparsity" % (module_name, p_name, str(p.size()), str(p.dtype)))
                        continue
    ...
    def prune_trained_model(cls, model, optimizer,mask_calculator="m8n7"):
        ...
        cls.init_model_for_pruning(model, mask_calculator=mask_calculator+'_1d', verbosity=3, whitelist=[torch.nn.Linear, torch.nn.Conv2d, torch.nn.MultiheadAttention], allow_recompute_mask=False)
