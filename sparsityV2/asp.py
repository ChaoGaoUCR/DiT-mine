...
def eligible_modules(model, whitelist_layer_types, allowed_layer_names, disallowed_layer_names, disallowed_modules="lora"):
    eligible_modules_list = []
    skip_modules = []
    for name, mod in model.named_modules():
        if isinstance(mod, whitelist_layer_types) and name not in disallowed_layer_names and disallowed_modules not in name:
            if allowed_layer_names is not None and name not in allowed_layer_names:
                continue
            eligible_modules_list.append((name, mod))
        if disallowed_modules in name:
...
class ASP:
    ...
    def init_model_for_pruning(cls, model, mask_calculator="m4n2_1d",
        ...
        if torchvision_imported:
            ...
            if torchvision_version_major == 0 and torchvision_version_minor < 12:
                sparse_parameter_list = {peft.tuners.lora.Linear: ['weight'] ,torch.nn.Linear: ['weight'], torch.nn.Conv1d: ['weight'], torch.nn.Conv2d: ['weight'], torch.nn.Conv3d: ['weight'], torch.nn.modules.linear.NonDynamicallyQuantizableLinear: ['weight'], torch.nn.MultiheadAttention: ['q_proj_weight', 'k_proj_weight', 'v_proj_weight', 'in_proj_weight'], torchvision.ops.misc.Conv2d: ['weight']}
            else:    # Torchvision remove APIs that were deprecated before 0.8 (#5386) in 0.12.0, torchvision.ops.misc.Conv2d is removed
                sparse_parameter_list = {peft.tuners.lora.Linear: ['weight'] ,torch.nn.Linear: ['weight'], torch.nn.Conv1d: ['weight'], torch.nn.Conv2d: ['weight'], torch.nn.Conv3d: ['weight'], torch.nn.modules.linear.NonDynamicallyQuantizableLinear: ['weight'], torch.nn.MultiheadAttention: ['q_proj_weight', 'k_proj_weight', 'v_proj_weight', 'in_proj_weight']}
        else:
            sparse_parameter_list = {peft.tuners.lora.Linear: ['weight'] ,torch.nn.Linear: ['weight'], torch.nn.Conv1d: ['weight'], torch.nn.Conv2d: ['weight'], torch.nn.Conv3d: ['weight'], torch.nn.modules.linear.NonDynamicallyQuantizableLinear: ['weight'], torch.nn.MultiheadAttention: ['q_proj_weight', 'k_proj_weight', 'v_proj_weight', 'in_proj_weight']}
        ...
        def add_sparse_attributes(module_name, module):
            ...
            # if ("peft.tuners.lora.Linear" == type(module)):
            #     breakpoint()
            ...
            for p_name, p in module.named_parameters():
                ...
                if p_name in sparse_parameters:
                    ...
                    if p.dtype == torch.float32 and ((p.size()[0] % 16) != 0 or (p.size()[1] % 16) != 0): #User defines FP32 and APEX internally uses FP16 math
                        print("[ASP] Auto skipping pruning %s::%s of size=%s and type=%s for sparsity" % (module_name, p_name, str(p.size()), str(p.dtype)))
                        continue
                    if p.dtype == torch.float16 and ((p.size()[0] % 16) != 0 or (p.size()[1] % 16) != 0): #For Conv2d dim= K x CRS; we prune along C
                        print("[ASP] Auto skipping pruning %s::%s of size=%s and type=%s for sparsity" % (module_name, p_name, str(p.size()), str(p.dtype)))
                        continue
    ...
    def compute_sparse_masks(cls):
        ...
        with torch.no_grad():
            ...
            #     try:
            #         successful_permutation = Permutation.permute_model(cls.__model.modules, dump_fx_graph=cls.__save_permutation_graph, save_dumped_fx_graph=os.path.join(cls.__permutation_output_dir, 'model_offline_permutation_graph.json'))
            ...
            for module_name, module, p_name, p, mask, pruned in cls.__sparse_parameters:
                ...
                mask.set_(cls.__calculate_mask(p, module.weight.grad))
