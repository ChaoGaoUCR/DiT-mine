...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if r > 0:
            ...
            self.lora_B = nn.Linear(r, out_features//2, bias=False)
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            if self.r > 0:
                # breakpoint()
                lora_weight = torch.matmul(self.lora_B.weight, self.lora_A.weight) 
                lora_weight_up = torch.zeros_like(self.weight, dtype=self.weight.dtype)
                zero_positions = mask.nonzero(as_tuple=True)
                lora_weight_up[zero_positions] = lora_weight.flatten()
                
                output = torch.matmul(x, lora_weight_up.t())* self.scaling
                result += output 

                # lora_output = self.lora_B(self.lora_A(self.lora_dropout(x.to(self.lora_A.weight.dtype)))) * self.scaling
                # output = torch.zeros_like(result, dtype=result.dtype)
                # zero_positions = mask.nonzero(as_tuple=True)
                # output[zero_positions] = lora_output.flatten()
                # result += output
        else:
             result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
